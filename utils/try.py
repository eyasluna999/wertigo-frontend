...
def extract_query_info(query_text, available_cities, available_categories, available_budgets=None):
    ...
    # Now call the function with appropriate parameters to extract budget
    if extracted_category:
        extracted_budget = classify_budget(query_lower, category=extracted_category.lower())
        # Also try to extract budget amount
        ...
        extracted_budget = classify_budget(query_lower)
...
def format_recommendations(recommendations, scores):
    ...
    for i, (idx, row) in enumerate(recommendations.iterrows()):
        ...
        rating = min(5, max(1, int(scores[i] * 5)))
